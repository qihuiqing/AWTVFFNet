...
class ResBlock(nn.Module): 
    ...
    def __init__(self, conv, n_feats, kernel_size, bias=True):
        ...
        self.convs1 = nn.Sequential(*[conv(n_feats, n_feats, kernel_size, bias = bias),
                                     bn(n_feats),
                                     act('ReLU')])
        
        self.convs2 = nn.Sequential(*[conv(n_feats, n_feats, kernel_size, bias = bias),
                                     bn(n_feats),
                                     act('ReLU')])
        self.un01 = nn.Conv2d(n_feats*3, n_feats, kernel_size = 3, padding=1, stride = 1)
        self.act = act('ReLU')

    def forward(self, x):
        res = self.convs1(x)
        res1 = self.convs2(res)
        out = torch.cat([x, res, res1], dim = 1)
        out = self.un01(out)
        out = self.act(out)
